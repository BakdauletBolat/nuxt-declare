        ...
        <div class="flex xl:gap-[70px] lg:gap-[40px]">
          <div class="lg:block hidden">
            <div class="flex justify-center mb-2">
              <img @click="prevThumbSlider" class="thumb-next-el cursor-pointer" src="@/assets/icons/arrow-top.svg"/>
            </div>
            <swiper @swiper="setThumbsSwiper"
                    ...
                    :modules="modules"
              class="thumb-slider">
              <swiper-slide v-for="image in product?.pictures.data"><img class="object-contain cursor-pointer " :src="image.attributes.url" /></swiper-slide>
            </swiper>
            <div class="flex justify-center mt-2">
              <img @click="nextThumbSlider" class="thumb-prev-el cursor-pointer" src="@/assets/icons/arrow-bottom.svg"/>
            </div>
          </div>
          <div class="flex w-full">
            <swiper v-if="thumbsSwiper" :pagination="pagination" @swiper="setMainSlider" :thumbs="{ swiper: thumbsSwiper }" :modules="modules"
              class="main-slider relative">
              <swiper-slide v-for="image in product?.pictures.data"><img class="object-contain" :src="image.attributes.url" /></swiper-slide>
      ...
      <div class="grid grid-cols-1 lg:gap-[40px] lg:grid-cols-2 gap-[28px] mt-[36px]">
        ...
        <div class="mt-[36px] lg:mt-[0px]">
          ...
          <div class="grid grid-cols-1">
            ...
            <FeatureItem v-if="product.collections.data.length > 0" keyItem="Коллекция" :value="product.collections.data.map(item=>item.attributes.title).join(', ')"></FeatureItem>
          </div>
        </div>
      </div>
    </div>
